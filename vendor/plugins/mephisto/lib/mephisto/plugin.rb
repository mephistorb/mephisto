module Mephisto
  ...
  class Plugin < ActiveRecord::Base
    ...
    class << self
      ...
      def default_options(values = nil)
        unless values.nil?
          option_accessors_source = values.collect do |property, default|
            <<-END
              def #{property}
          ...
          class_eval option_accessors_source * "\n", __FILE__, __LINE__
      ...
      #     module Plugins
      #       class Foo < Mephisto::Plugin
      #         public_controller config, directory, 'Foo', 'foo'
      #       end
      #     end
      #   end
      #
      ...
      def public_controller(config, directory, title, name = nil)
        returning((name || title.underscore).to_sym) do |controller_name|
          config.controller_paths << File.join(directory, 'lib')
          view_paths[controller_name] = File.join(directory, 'views')
        end
      end
      ...
      #     module Plugins
      #       class Foo < Mephisto::Plugin
      #         admin_controller config, directory, 'Foo', 'foo'
      #       end
      #     end
      #   end
      #
      ...
      def admin_controller(config, directory, title, name = nil, options = {})
        returning public_controller(config, directory, title, name) do |controller_name|
      ...
      protected
        def find_plugin_migration
          plugin_name = "mephisto_#{plugin_name}"
          require(RAILS_PATH + 'vendor/plugins' + plugin_name + 'lib' + plugin_name + 'schema')
          def self.find_plugin_migration; self.const_get :Schema; end
          find_plugin_migration
        end
    end

    plugin_property_source = %w(desc author version homepage plugin_name default_options).collect! do |property|
