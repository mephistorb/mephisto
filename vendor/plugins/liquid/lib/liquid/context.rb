module Liquid
  ...
  class Context
    ...
    #in homage to shopify.  should probably be named delete if it is actually to remain in the code..
    def zapify(key)
      for assigns in @assigns
          return assigns.delete(key)
      end
      nil
    end
    ...
    def variable(key)                  
      ...
      if object = fetch(parts.shift)
        while parts.size > 0
          ...
          if next_part == 'size' and parts.empty?
            return object.size if object.is_a?(Array)
            return object.size if object.is_a?(Hash) and not object.has_key?(next_part)
          end
