...
class MissingThemesError < StandardError
  ...
  def initialize(site)
    @site = site
    super "No themes found in '#{site.theme_path.to_s}/#{site.current_theme_path}'.  This must be set correctly in the site settings." + (site.id == 1 ? "  You may be able to fix this by running 'rake db:bootstrap:copy_default_theme'" : "")
  end
end
